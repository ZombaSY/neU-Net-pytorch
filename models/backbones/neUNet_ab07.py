...
# TODO: implement each module in subclass
...
class neUNet_ab07(nn.Module):
    def __init__(self, channel, n_classes=1, base_c=64, depths=[1, 1, 1, 1], kernel_size=7):
        ...
        self.up_conv3 = nn.ConvTranspose2d(base_c * 8, base_c * 8, kernel_size=2, stride=2)
        self.up_residual_conv3 = ResidualConv(base_c * 8, base_c * 4, 1, 1)

        self.up_conv2 = nn.ConvTranspose2d(base_c * 4, base_c * 4, kernel_size=2, stride=2)
        self.up_residual_conv2 = ResidualConv(base_c * 4, base_c * 2, 1, 1)

        self.up_conv1 = nn.ConvTranspose2d(base_c * 2, base_c * 2, kernel_size=2, stride=2)
        ...
        self.conv_cat_1 = M_Conv(base_c * 16 + base_c * 4 + base_c * 2, base_c * 2, kernel_size=1)
        self.conv_cat_2 = M_Conv(base_c * 16 + base_c * 4, base_c * 4, kernel_size=1)
        self.conv_cat_3 = M_Conv(base_c * 16, base_c * 8, kernel_size=1)
        self.attention1 = Attention_block(F_g=base_c * 2, F_l=base_c * 2, F_int=base_c * 1)
        self.attention2 = Attention_block(F_g=base_c * 4, F_l=base_c * 4, F_int=base_c * 2)
        self.attention3 = Attention_block(F_g=base_c * 8, F_l=base_c * 8, F_int=base_c * 4)

    def load_pretrained(self, dst):     # TODO: remove in release
        pretrained_states = torch.load(dst)
        pretrained_states_new = OrderedDict()

        for item in pretrained_states.keys():
            if 'residual_conv_1' in item:
                pretrained_states_new[item.replace('residual_conv_1', 'down_conv_2')] = pretrained_states[item]
            elif 'residual_conv_2' in item:
                pretrained_states_new[item.replace('residual_conv_2', 'down_conv_3')] = pretrained_states[item]
            elif 'bridge' in item:
                pretrained_states_new[item.replace('bridge', 'down_conv_4')] = pretrained_states[item]
            elif 'up_residual_conv3' in item:
                pretrained_states_new[item.replace('up_residual_conv3', 'up_residual_conv1')] = pretrained_states[item]
            elif 'up_residual_conv1' in item:
                pretrained_states_new[item.replace('up_residual_conv1', 'up_residual_conv3')] = pretrained_states[item]
            elif 'upsample_' in item:
                pass
            else:
                pretrained_states_new[item] = pretrained_states[item]

        pretrained_states_new2 = OrderedDict()

        for item in pretrained_states_new.keys():
            item_new = item.replace('module.unext.', '')

            if 'down_conv_' in item and 'pwconv1' in item:
                pass
            elif 'input_layer' in item and 'pwconv1' in item:
                pass
            else:
                pretrained_states_new2[item_new] = pretrained_states_new[item]

        self.load_state_dict(pretrained_states_new2)
    ...
    def forward(self, x):
        ...
        x4 = self.up_conv3(x4)
        x3_up = self.attention3(x4, x3_gf_conv)
        x3_up = self.up_residual_conv3(x3_up)

        _, _, h, w = x2_down.size()
        x2_gf = torch.cat([x2_down, F.interpolate(x3_gf, size=(h, w), mode='bilinear', align_corners=True)], dim=1)
        x2_gf_conv = self.conv_cat_2(x2_gf)
        x2_up = self.up_conv2(x3_up)
        x2_up = self.attention2(x2_up, x2_gf_conv)
        x2_up = self.up_residual_conv2(x2_up)

        _, _, h, w = x1_down.size()
        x1_gf = torch.cat([x1_down, F.interpolate(x2_gf, size=(h, w), mode='bilinear', align_corners=True)], dim=1)
        x1_gf_conv = self.conv_cat_1(x1_gf)
        x1_up = self.up_conv1(x2_up)
        x1_up = self.attention1(x1_up, x1_gf_conv)
        x1_up = self.up_residual_conv1(x1_up)
