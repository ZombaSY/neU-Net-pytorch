...
class ResidualConv(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size, padding, stride):
        super(ResidualConv, self).__init__()

        self.conv_block = nn.Sequential(
            nn.BatchNorm2d(input_dim),
            nn.ReLU(),
            nn.Conv2d(input_dim, output_dim, kernel_size=kernel_size, padding=padding, stride=stride),
            nn.BatchNorm2d(output_dim),
            nn.ReLU(),
            nn.Conv2d(output_dim, output_dim, kernel_size=kernel_size, padding=padding, stride=stride),
        )
...
class neUNet_ab08(nn.Module):
    def __init__(self, channel, n_classes=1, base_c=64, depths=[1, 1, 1, 1], kernel_size=3):
        super(neUNet_ab08, self).__init__()

        self.input_layer = nn.Sequential(
            M_Conv(channel, base_c * 1, kernel_size=kernel_size),
            *[ResidualConv(base_c * 1, base_c * 1, kernel_size=kernel_size, padding=1, stride=1) for _ in range(depths[0])]
        )
        ...
        self.down_conv_2 = nn.Sequential(*[
            ...
            *[ResidualConv(base_c * 2, base_c * 2, kernel_size=kernel_size, padding=1, stride=1) for _ in range(depths[1])]
            ])
        ...
        self.down_conv_3 = nn.Sequential(*[
            ...
            *[ResidualConv(base_c * 4, base_c * 4, kernel_size=kernel_size, padding=1, stride=1) for _ in range(depths[2])]
            ])
        ...
        self.down_conv_4 = nn.Sequential(*[
            ...
            *[ResidualConv(base_c * 8, base_c * 8, kernel_size=kernel_size, padding=1, stride=1) for _ in range(depths[3])]
            ])
        self.attn = SelfAttentionBlock()

        self.up_residual_conv3 = ResidualConv(base_c * 8, base_c * 4, kernel_size=kernel_size, padding=1, stride=1)
        self.up_residual_conv2 = ResidualConv(base_c * 4, base_c * 2, kernel_size=kernel_size, padding=1, stride=1)
        self.up_residual_conv1 = ResidualConv(base_c * 2, base_c * 1, kernel_size=kernel_size, padding=1, stride=1)
    ...
    def forward(self, x):
        ...
        # Decoder TODO: concatenate up_conv vs x{n}_gf_conv vs UNeXt_rev41
